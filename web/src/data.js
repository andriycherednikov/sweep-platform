/* ============================================================
   THE SWEEP — sample dataset (deterministic, believable)
   Exports SWEEP = { teams, people, ownership, fixtures,
   standings, photos, derbies, money, fmt, helpers }
   ============================================================ */
...
function ownersForFixture(f){ return { t1: ownersOf(f.t1), t2: ownersOf(f.t2) }; }

// ---- "in the money" board: rank people by best owned team's projected finish ----
// projected: from strength -> title odds-ish
Object.keys(teams).forEach(function (c) {
  var s = teams[c].strength;
  teams[c].titleOdds = Math.max(1, Math.round(Math.pow(Math.max(0, s - 50), 2) / 14));
  teams[c].outlook = s >= 86 ? "Title contender" : s >= 80 ? "Last-8 shout" : s >= 73 ? "Knockout dark horse" : s >= 66 ? "Group toss-up" : "Long shot";
});
const money = people.map(function (p) {
  var best = p.teams.map(function (c) { return teams[c]; }).sort(function (a, b) { return b.strength - a.strength; })[0];
  return { person: p, team: best, odds: best.titleOdds, strength: best.strength };
}).sort(function (a, b) { return b.strength - a.strength; });
money.forEach(function (m, i) {
  m.rank = i + 1;
  m.tag = i === 0 ? "Title fav" : m.strength >= 80 ? "Alive" : m.strength >= 70 ? "Alive" : "Outside";
});
...
fixtures.forEach(function (f) {
  f.timeLabel = fmtTime(f.ko);
  f.dayLabel = fmtDay(f.ko);
  f.dayKey = fmtDayKey(f.ko);
});
...
export const SWEEP = {
  teams: teams, teamList: Object.keys(teams).map(function (c) { return teams[c]; }),
  groups: Object.keys(GROUPS),
  people: people, fixtures: fixtures, standings: standings,
  photos: photos, derbies: derbies, money: money,
  nextMatch: derbyMatch, liveMatch: liveMatch,
  team: team, flag: flag, gd: gd,
  ownersOf: ownersOf, ownersForFixture: ownersForFixture,
  fmtTime: fmtTime, fmtDay: fmtDay, fmtDayKey: fmtDayKey, fmtWeekday: fmtWeekday,
  todayKey: fmtDayKey(new Date("2026-06-13T08:00:00Z"))
};
