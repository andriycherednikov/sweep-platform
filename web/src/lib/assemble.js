...
export function assembleSweep(api) {
  ...
  const fixtures = rawFixtures.map((f) => {
    ...
    // penScore comes only from the provider's score.penalty (a real shootout). Do NOT
    // derive it from minute-120 penalty events — an extra-time penalty converted at 120'
    // is an in-play goal, not a shootout kick, and would fabricate a phantom tally.
    const penScore = f.penScore ?? null;
