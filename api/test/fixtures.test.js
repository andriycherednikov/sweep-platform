...
test('GET /api/fixtures returns all fixtures ordered by kickoff', async () => {
  const res = await client.inject({ method: 'GET', url: '/api/fixtures' })
...
test('GET /api/fixtures?team=hr returns only Croatia matches', async () => {
  const res = await client.inject({ method: 'GET', url: '/api/fixtures?team=hr' })
  const list = res.json()
...
test('GET /api/fixtures?person=p4 returns Andriy\'s teams\' matches', async () => {
  const res = await client.inject({ method: 'GET', url: '/api/fixtures?person=p4' })
  const list = res.json()
...
test('fixtures carry a lineups field (null by default)', async () => {
  const list = (await client.inject({ method: 'GET', url: '/api/fixtures' })).json()
...
test('GET /api/fixtures/:id returns one fixture or 404', async () => {
  const all = (await client.inject({ method: 'GET', url: '/api/fixtures' })).json()
  const one = await client.inject({ method: 'GET', url: `/api/fixtures/${all[0].id}` })
  expect(one.statusCode).toBe(200)
  expect(one.json().id).toBe(all[0].id)
  const missing = await client.inject({ method: 'GET', url: '/api/fixtures/nope' })
