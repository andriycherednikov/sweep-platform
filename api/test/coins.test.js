...
import { person, coinLedger, bet, event, parlay } from '../src/db/schema.js'
...
test('GET /api/coins?personId= returns that person balance after their grant', async () => {
  ...
  const body = (await client.inject({ method: 'GET', url: `/api/coins?personId=${p.id}` })).json()
...
test('GET /api/coins with an unknown personId returns an empty wallet, not a DB error', async () => {
  const res = await client.inject({ method: 'GET', url: '/api/coins?personId=does_not_exist' })
  expect(res.statusCode).toBe(200)
  const body = res.json()
...
const balanceOfPerson = async (id) => (await client.inject({ method: 'GET', url: `/api/coins?personId=${id}` })).json().balance
...
test('POST /api/bet deducts the stake, locks the odds, and returns the new balance', async () => {
  ...
  const res = await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, selection: 'HOME', stake: 100 } })
  expect(res.statusCode).toBe(200)
  const body = res.json()
...
test('POST /api/bet places an over/under bet with its line', async () => {
  ...
  const res = await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, market: 'ou25', selection: 'OVER', stake: 50 } })
  expect(res.statusCode).toBe(200)
...
test('POST /api/bet rejects an unknown market or selection', async () => {
  const p = await aPerson(); const f = await bettableFixture()
  expect((await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, market: 'cards', selection: 'OVER', stake: 10 } })).json()).toEqual({ error: 'no_odds' })
  expect((await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, market: '1x2', selection: 'ZZZ', stake: 10 } })).json()).toEqual({ error: 'no_odds' })
})

test('POST /api/bet rejects a stake above the balance', async () => {
  const p = await aPerson(); const f = await bettableFixture()
  const res = await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, selection: 'HOME', stake: 99999999 } })
...
test('POST /api/bet now accepts knockout fixtures (full tournament) and still rejects an unpriced fixture', async () => {
  ...
  const ok = await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, selection: 'HOME', stake: 10 } })
  ...
  expect((await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, selection: 'HOME', stake: 10 } })).json()).toEqual({ error: 'no_odds' })
})

test('POST /api/bet allows multiple independent bets on the same match', async () => {
  const p = await aPerson(); const f = await bettableFixture()
  const before = await balanceOfPerson(p.id)
  await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, selection: 'HOME', stake: 50 } })
  const second = await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, selection: 'AWAY', stake: 50 } })
  expect(second.statusCode).toBe(200)
  const wallet = (await client.inject({ method: 'GET', url: `/api/coins?personId=${p.id}` })).json()
...
test('GET /api/coins returns parlays and excludes parlay legs from single bets', async () => {
  const p = await aPerson(); const f = await bettableFixture()
  await balanceOfPerson(p.id)
  await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, selection: 'HOME', stake: 10 } })
...
test('POST /api/parlay places a 2-leg parlay: combined odds × stake, two legs, one debit', async () => {
  ...
  const before = await balanceOfPerson(p.id)
  const res = await client.inject({ method: 'POST', url: '/api/parlay', payload: { personId: p.id, stake: 100, legs: [
...
test('POST /api/parlay validation: too few legs, duplicate market, leg errors, minor, funds', async () => {
  const p = await aPerson(); const [f1, f2] = await twoBettableFixtures()
  await balanceOfPerson(p.id)
  const post = (payload) => client.inject({ method: 'POST', url: '/api/parlay', payload })
...
test('two concurrent full-balance bets cannot overdraw — exactly one wins', async () => {
  ...
  const [a, b] = await Promise.all([
    client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, selection: 'HOME', stake: before } }),
    client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, selection: 'AWAY', stake: before } }),
  ])
...
test('GET /api/coins/ledger returns the grant entry with a running balance', async () => {
  const p = await aPerson()
  const res = await client.inject({ method: 'GET', url: `/api/coins/ledger?personId=${p.id}` })
  expect(res.statusCode).toBe(200)
  const body = res.json()
  expect(body.balance).toBeGreaterThanOrEqual(1000)
...
test('GET /api/coins/ledger reflects a placed bet as a stake entry with its bet attached', async () => {
  const p = await aPerson(); const f = await bettableFixture()
  await balanceOfPerson(p.id) // seed grant
  await client.inject({ method: 'POST', url: '/api/bet', payload: { fixtureId: f.id, personId: p.id, selection: 'HOME', stake: 100 } })
  const body = (await client.inject({ method: 'GET', url: `/api/coins/ledger?personId=${p.id}` })).json()
...
test('GET /api/coins/ledger with an unknown personId returns an empty statement, not an error', async () => {
  const res = await client.inject({ method: 'GET', url: '/api/coins/ledger?personId=does_not_exist' })
  expect(res.statusCode).toBe(200)
...
test('GET /api/coins/ledger attaches the parlay to its stake entry', async () => {
  const p = await aPerson(); const [f1, f2] = await twoBettableFixtures()
  await balanceOfPerson(p.id)
  await client.inject({ method: 'POST', url: '/api/parlay', payload: { personId: p.id, stake: 50, legs: [
